...
@section('title')
    @include('layouts.inc.title',['title'=>"Service - Requirements"])
@endsection
...
@section('content')
    ...
    <div class="container">
        <div class="d-flex justify-content-between flex-wrap flex-md-nowrap align-items-center pt-3 pb-2 mb-3 border-bottom">
            <h1 class="h2"> REQUIREMENTS
                <a class="btn btn-primary" href="{{route('employee.services.requirements.add',['id'=>$service->id])}}">
                    ...
                    <table id="sortedTable" class="table table-striped table-sm">
                        ...
                        @foreach($requirements as $requirement)
                            <tr>
                                ...
                                <td>{{$requirement->requirement_desc}}</td>
                                <td>{{$requirement->wheretosecure}}</td>

                                <td class="text-center">
                                    <a class="btn btn-success btn-sm"
                                       href="{{route('employee.services.requirements.edit',['id'=>$requirement->id])}}"><i
                                                class="fas fa-edit"></i> Edit</a>

                                    <a class="btn btn-danger btn-sm"
                                       href="{{route('employee.services.requirements.deleterequirement',['id'=>$requirement->id])}}"><i
